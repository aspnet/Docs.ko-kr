...
1. `wwwroot/index.html`의 경우

   * 범위가 지정 된 `<link>` 스타일의 요소에 요소를 추가 `<head>` 합니다.
   * 구성 요소를 로드 하는 요소를 변경 `App` 합니다.
...
1. 에서 `Shared/MainLayout.razor` 요소의 모든 콘텐츠를 `<div>` 의로 묶습니다 `id` `page` .
...
1. 최신 기본 파일에는 `wwwroot/css/app.css` 다음 스타일이 포함 됩니다. 다음 스타일과 앱에 추가한 모든 스타일의 추가 스타일을 제거 합니다.

    `wwwroot/css/app.css` (개발자가 추가한 스타일을 포함 하지 않는 기본 스타일):
