...
## <a name="changes-to-no-locblazor-app-routing-logic-in-501"></a>Blazor5.0.1 용의 앱 라우팅 논리 변경 내용
...
`PreferExactMatches`가로 설정 된 경우 `@true` 경로 일치는 와일드 카드를 통한 정확한 일치를 선호 합니다.

> [!IMPORTANT]
> 모든 앱은 명시적으로로 설정 되어야 합니다 `PreferExactMatches` `@true` .
>
> `PreferExactMatches` `@false` C #에서 옵션을 기본값으로 설정 하거나 설정 하지 않은 상태로 유지 하는 기능은 `false` *이전 버전과의 호환성을 위해서만 제공 됩니다*.
>
