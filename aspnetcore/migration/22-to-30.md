...
### <a name="use-newtonsoftjson-in-an-aspnet-core-30-signalr-project"></a>ASP.NET Core 3.0 프로젝트에서 Newtonsoft.Js사용 SignalR
...
ASP.NET Core 3.0에서 ASP.NET Core MVC는 `Async` 컨트롤러 작업 이름에서 접미사를 제거 합니다. 라우팅 및 링크 생성은 모두이 새 기본값의 영향을 받습니다. 예를 들어:
...
* 접미사를 지정 하는 데 필요한 링크 생성 `Async` 예를 들어:
...
* 링크 생성에는 접미사를 지정할 필요가 없습니다 `Async` . 예를 들어:
...
[이전 라우팅 버전의 차이점](xref:fundamentals/routing#differences-from-earlier-versions-of-routing)에 대 한 설명서에 설명 된 것 처럼 링크 생성에는 몇 가지 차이점이 있습니다 ( `Url.Link` 예: 및 유사한 api 사용). 추가 설정은 다음과 같습니다.
