...
ASP.NET Core 3.0에는 특성 [](https://www.owasp.org/index.php/SameSite) `SameSiteMode` `Unspecified` 쓰기를 억제 하는 특성 값을 포함 하는 SameSite 특성에 대 한 지원이 기본적으로 제공 됩니다.
...
Startup.cs의 메서드에서 파이프라인 사용에 삽입 `app.UseCookiePolicy()` `Configure(IApplicationBuilder, IHostingEnvironment)` 합니다. [](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/AspNetCore21MVC/Startup.cs) 예
...
* `SameSite`가로 설정 되어 `None` 있고 현재 사용자 에이전트가 none 특성 값을 지원 하지 않는 것으로 알려진 경우 확인은 [SameSiteSupport](https://github.com/dotnet/AspNetCore.Docs/blob/master/aspnetcore/security/samesite/snippets/SameSiteSupport.cs) 클래스를 사용 하 여 수행 됩니다.
