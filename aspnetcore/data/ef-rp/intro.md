...
* <span data-ttu-id="c3e96-426">다음 명령을 실행하여 :::no-loc(Razor)::: Pages 프로젝트를 만들고 `cd`를 사용하여 새 프로젝트 폴더로 이동합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-426">Run the following commands to create a :::no-loc(Razor)::: Pages project and `cd` into the new project folder:</span></span>
...
* <span data-ttu-id="c3e96-488">`Student` 엔터티에 대한 CRUD(만들기, 읽기, 업데이트 및 삭제) 작업을 처리하는 :::no-loc(Razor)::: 페이지.</span><span class="sxs-lookup"><span data-stu-id="c3e96-488">:::no-loc(Razor)::: pages that handle Create, Read, Update, and Delete (CRUD) operations for the `Student` entity.</span></span>
...
* <span data-ttu-id="c3e96-492">**스캐폴드 추가** 대화 상자에서 **Entity Framework를 사용한 :::no-loc(Razor)::: Pages(CRUD)** > **추가** 를 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-492">In the **Add Scaffold** dialog, select **:::no-loc(Razor)::: Pages using Entity Framework (CRUD)** > **ADD**.</span></span>
* <span data-ttu-id="c3e96-493">**Entity Framework(CRUD)를 사용한 :::no-loc(Razor)::: Pages 추가** 대화 상자에서 다음 작업을 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-493">In the **Add :::no-loc(Razor)::: Pages using Entity Framework (CRUD)** dialog:</span></span>
...
* <span data-ttu-id="c3e96-510">*Pages/Students* 폴더에서 다음 :::no-loc(Razor)::: 페이지를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-510">Creates :::no-loc(Razor)::: pages in the *Pages/Students* folder:</span></span>
...
* <span data-ttu-id="c3e96-518">*:::no-loc(appsettings.json):::* 에 데이터베이스 연결 문자열을 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-518">Adds a database connection string to *:::no-loc(appsettings.json):::*.</span></span>
...
<span data-ttu-id="c3e96-521">*:::no-loc(appsettings.json):::* 파일은 연결 문자열 [SQL Server LocalDB](/sql/database-engine/configure-windows/sql-server-2016-express-localdb)를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-521">The *:::no-loc(appsettings.json):::* file specifies the connection string [SQL Server LocalDB](/sql/database-engine/configure-windows/sql-server-2016-express-localdb).</span></span>
...
<span data-ttu-id="c3e96-538">:::no-loc(Razor)::: Pages 코드가 새 DBSet 이름과 일치하게 만들려면 전체 프로젝트에서 `_context.Student`를 `_context.Students`로 전체 변경합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-538">To make the :::no-loc(Razor)::: Pages code match the new DBSet name, make a global change across the whole project of `_context.Student` to `_context.Students`.</span></span>  <span data-ttu-id="c3e96-539">8개 발생 항목이 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-539">There are 8 occurrences.</span></span>
...
<span data-ttu-id="c3e96-542">ASP.NET Core는 [종속성 주입](xref:fundamentals/dependency-injection)을 사용하여 빌드됩니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-542">ASP.NET Core is built with [dependency injection](xref:fundamentals/dependency-injection).</span></span> <span data-ttu-id="c3e96-543">서비스(예: EF Core 데이터베이스 컨텍스트)는 애플리케이션 시작 중에 종속성 주입에 등록됩니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-543">Services (such as the EF Core database context) are registered with dependency injection during application startup.</span></span> <span data-ttu-id="c3e96-544">이러한 서비스(예: :::no-loc(Razor)::: Pages)가 필요한 구성 요소는 생성자 매개 변수를 통해 해당 서비스를 제공받습니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-544">Components that require these services (such as :::no-loc(Razor)::: Pages) are provided these services via constructor parameters.</span></span> <span data-ttu-id="c3e96-545">데이터베이스 컨텍스트 인스턴스를 가져오는 생성자 코드는 자습서 뒷부분에 나옵니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-545">The constructor code that gets a database context instance is shown later in the tutorial.</span></span>
...
<span data-ttu-id="c3e96-625">Contoso University 샘플 웹앱은 EF(Entity Framework) Core를 사용하여 ASP.NET Core :::no-loc(Razor)::: Pages 앱을 만드는 방법을 보여줍니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-625">The Contoso University sample web app demonstrates how to create an ASP.NET Core :::no-loc(Razor)::: Pages app using Entity Framework (EF) Core.</span></span>
...
<span data-ttu-id="c3e96-634">[:::no-loc(Razor)::: Pages](xref:razor-pages/index) 숙련도.</span><span class="sxs-lookup"><span data-stu-id="c3e96-634">Familiarity with [:::no-loc(Razor)::: Pages](xref:razor-pages/index).</span></span> <span data-ttu-id="c3e96-635">신규 프로그래머는 이 시리즈를 시작하기 전에 [:::no-loc(Razor)::: 페이지 시작하기](xref:tutorials/razor-pages/razor-pages-start)를 먼저 완료해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-635">New programmers should complete [Get started with :::no-loc(Razor)::: Pages](xref:tutorials/razor-pages/razor-pages-start) before starting this series.</span></span>
...
<span data-ttu-id="c3e96-645">이 사이트의 UI 스타일은 기본 제공 템플릿에서 생성된 것과 가깝습니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-645">The UI style of this site is close to what's generated by the built-in templates.</span></span> <span data-ttu-id="c3e96-646">이 자습서에서는 UI가 아닌 :::no-loc(Razor)::: 페이지를 사용한 EF Core를 중점적으로 다룹니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-646">The tutorial focus is on EF Core with :::no-loc(Razor)::: Pages, not the UI.</span></span>

## <a name="create-the-contosouniversity-no-locrazor-pages-web-app"></a><span data-ttu-id="c3e96-647">ContosoUniversity :::no-loc(Razor)::: Pages 웹앱 만들기</span><span class="sxs-lookup"><span data-stu-id="c3e96-647">Create the ContosoUniversity :::no-loc(Razor)::: Pages web app</span></span>
...
<span data-ttu-id="c3e96-654">이전 단계의 이미지는 [:::no-loc(Razor)::: 웹앱 만들기](xref:tutorials/razor-pages/razor-pages-start#create-a-razor-pages-web-app)를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="c3e96-654">For images of the preceding steps, see [Create a :::no-loc(Razor)::: web app](xref:tutorials/razor-pages/razor-pages-start#create-a-razor-pages-web-app).</span></span>
...
* <span data-ttu-id="c3e96-727">**스캐폴드 추가** 대화 상자에서 **Entity Framework를 사용한 :::no-loc(Razor)::: Pages(CRUD)** > **추가** 를 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-727">In the **Add Scaffold** dialog, select **:::no-loc(Razor)::: Pages using Entity Framework (CRUD)** > **ADD**.</span></span>

<span data-ttu-id="c3e96-728">**Entity Framework(CRUD)를 사용하여 :::no-loc(Razor)::: Pages 추가** 대화 상자를 완료합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-728">Complete the **Add :::no-loc(Razor)::: Pages using Entity Framework (CRUD)** dialog:</span></span>
...
* <span data-ttu-id="c3e96-743">*:::no-loc(appsettings.json):::* : 로컬 데이터베이스에 연결하는 데 사용된 연결 문자열이 추가됩니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-743">*:::no-loc(appsettings.json):::* : The connection string used to connect to a local database is added.</span></span>
...
<span data-ttu-id="c3e96-745">ASP.NET Core는 [종속성 주입](xref:fundamentals/dependency-injection)을 사용하여 빌드됩니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-745">ASP.NET Core is built with [dependency injection](xref:fundamentals/dependency-injection).</span></span> <span data-ttu-id="c3e96-746">서비스(예: EF Core DB 컨텍스트)는 애플리케이션 시작 중에 종속성 주입에 등록됩니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-746">Services (such as the EF Core DB context) are registered with dependency injection during application startup.</span></span> <span data-ttu-id="c3e96-747">이러한 서비스(예: :::no-loc(Razor)::: Pages)가 필요한 구성 요소는 생성자 매개 변수를 통해 해당 서비스를 제공받습니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-747">Components that require these services (such as :::no-loc(Razor)::: Pages) are provided these services via constructor parameters.</span></span> <span data-ttu-id="c3e96-748">DB 컨텍스트 인스턴스를 가져오는 생성자 코드는 자습서 뒷부분에 나옵니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-748">The constructor code that gets a db context instance is shown later in the tutorial.</span></span>
...
<span data-ttu-id="c3e96-773">앱을 실행하고 :::no-loc(cookie)::: 정책에 동의합니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-773">Run the app and accept the :::no-loc(cookie)::: policy.</span></span> <span data-ttu-id="c3e96-774">이 앱은 개인 정보를 보관하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-774">This app doesn't keep personal information.</span></span> <span data-ttu-id="c3e96-775">[EU GDPR(일반 데이터 보호 규정) 지원](xref:security/gdpr)에서 :::no-loc(cookie)::: 정책을 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c3e96-775">You can read about the :::no-loc(cookie)::: policy at [EU General Data Protection Regulation (GDPR) support](xref:security/gdpr).</span></span>
