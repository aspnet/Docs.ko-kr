...
[Blazor Server 프로젝트 템플릿](xref:blazor/project-structure)을 기반으로 하는 앱의 `Counter` 구성 요소에서 `currentCount` 값을 유지하려면 `ProtectedSessionStore.SetAsync`를 사용하도록 `IncrementCount` 메서드를 수정합니다.
