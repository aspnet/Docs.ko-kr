...
이 문제를 해결하기 위해 Blazor는 임의로 커질 수 있는 목록의 모양 및 스크롤 동작을 만들지만 현재 스크롤 뷰포트에 있는 목록 항목만 렌더링하는 `Virtualize` 구성 요소를 제공합니다. 예를 들어 이는 앱은 100,000개 항목을 포함하는 목록이 있지만 한 번에 표시되는 20개 항목의 렌더링 비용만 지급할 수 있음을 의미합니다. `Virtualize` 구성 요소를 사용하여 크기 순서별로 UI 성능을 스케일 업할 수 있습니다.
